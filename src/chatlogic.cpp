...
ChatLogic::~ChatLogic() {
  ...
  // delete all nodes
  // No need to delete _nodes as unique_ptr handles it
  /*
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename) {
          ...
          if (type->second == "NODE") {
            ...
            if (newNode == _nodes.end()) {
              _nodes.emplace_back(std::make_unique<GraphNode>(
                  id));  // Use make_unique for memory management
              newNode = _nodes.end() - 1;  // get iterator to last element

              // add all answers to current node
              AddAllTokensToElement("ANSWER", tokens,
                                    *(*newNode));  // Dereference unique_ptr
            }
          }

          // edge-based processing
          if (type->second == "EDGE") {
            // find tokens for incoming (parent)
            // and outgoing (child) node
  ...
  _chatBot = new ChatBot(chatbot);  // Copy constructor
